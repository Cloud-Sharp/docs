...
즉, 이미 확정된 사용량과 현재 예약된 용량, 그리고 새 업로드가 추가로 점유할 예상 용량의 합이 사용자 허용 용량 이하일 때만 업로드를 시작할 수 있다.
...
`storage_allowed_bytes = NULL`인 사용자는 **무제한 사용자**로 간주한다.  
무제한 사용자의 경우 quota 초과 판정은 수행하지 않으며, 업로드 시작 시 용량 부족으로 거절하지 않는다.

다만 무제한 사용자라 하더라도 다음 값들은 계속 관리한다.
...
### 3. 동일 사용자 다중 세션 동시 업로드 경쟁 방지

동일 사용자가 여러 탭 또는 여러 클라이언트에서 동시에 업로드를 시작할 수 있으므로, quota 판정과 예약 증가 처리는 반드시 **단일 트랜잭션 내 원자적 작업**으로 수행해야 한다.
...
즉, 동일 사용자에 대한 다중 업로드 경쟁은 **“quota 판정 + reserved 증가”를 한 트랜잭션으로 묶는 방식**으로 방지한다.
...
1. `FileItem` 생성
2. `User.storage_used_bytes += final_size`
3. `User.storage_reserved_bytes -= reserved_size`
...
- `User.storage_reserved_bytes -= reserved_size`
...
따라서 운영 배치나 정합성 점검 작업을 통해 `User` 집계 캐시와 실제 `FileItem`, `FileReservation` 합계를 주기적으로 대조할 수 있어야 한다.