...
flowchart TD
    ...
    P --> Q["DB 트랜잭션<br/>FileItem 생성<br/>사용량 반영<br/>상태 완료 처리"]
    Q --> R[후처리 큐 등록]
    R --> S([클라이언트: 완료 결과 수신])
...
> **참고**: 중복 파일명은 기존 `FileItem`뿐 아니라, 현재 활성 상태인 `FileReservation`도 함께 검사한다. 이 단계의 검사는 **사전 검사**이며, 최종 확정 시 동일 항목을 재검증할 수 있다.
...
flowchart TD
    ...
    B --> C["quota 용량 선점<br/>storage_reserved_bytes += expectedSize"]
...
| quota 예정 용량 | 동시 업로드 시 quota 경쟁 조건 방지 |
...
| 작업 | 설명 |
|------|------|
| `storage_used_bytes` 증가 | 사용자 실사용량 반영 |
...
flowchart TD
    ...
    subgraph 트랜잭션_내부["🔒 DB 트랜잭션"]
        C[FileItem 생성]
        D["User.storage_used_bytes 증가"]
        E["User.storage_reserved_bytes 차감"]
