...
flowchart TD
    ...
    A -->|Authorization 헤더| B[opaque session Bearer 토큰 검증]
...
flowchart LR
    subgraph 인증 사용자
        A1["Authorization: Bearer {opaque_session_token}"] --> A2[/"subjectType = user
        subjectId = {userId}"/]
    end
...
| 접근 경로     | 인증 방식                             | 정규화 결과                                              |
| --------- | --------------------------------- | --------------------------------------------------- |
| 인증 사용자    | `Authorization` 헤더의 opaque session Bearer 토큰 | `subjectType=user`, `subjectId={userId}`            |
...
| 상황               | 응답                 | 이유              |
| ---------------- | ------------------ | --------------- |
| 사용자 세션 토큰 유효하지 않음 / 만료 | `401 Unauthorized` | 인증 실패는 명시적으로 알림 |
...
| 보호 방식 | 원문 토큰은 한 번만 반환하고, 서버에는 `session_token_hash`와 세션 메타데이터 저장 |
...
| # | 항목 | 관련 단계 | 중요도 |
|---|------|-----------|--------|
| 1 | opaque session token / share token 검증 | 1단계 | 🔴 Critical |
