...
Outbox에 기록된 도메인 이벤트를 인증된 브라우저 클라이언트에게 Server-Sent Events(SSE)로 실시간 전달한다. 서버 연결 저장소는 단일 API 인스턴스 메모리만 사용한다. Worker Pub/Sub dispatcher는 구현되어 있지만, API 인스턴스 간 SSE fan-out용 Redis Pub/Sub/Streams 계층은 아직 없다.

## 2. 가정

- 클라이언트 탭당 1개 SSE 연결을 전제한다.
- SSE 전달은 best-effort live update다. 연결이 없거나 channel overflow가 나면 DB outbox 재전송 보장을 추가하지 않는다.
- 서버 메모리 저장소는 단일 API 인스턴스 기준이다. 여러 API 인스턴스 SSE fan-out은 이후 Redis Pub/Sub/Streams로 확장한다.
- Outbox는 `RealtimeFanout`, `NotificationProjection`, `Worker` target을 모두 지원한다. 이 문서는 그중 SSE delivery인 `RealtimeFanout`을 중심으로 설명한다.
...
| `NotificationProjectionOutboxEventDispatcher` | `CloudSharp.Api.Outbox` | `Target = NotificationProjection`. SSE와 같은 outbox envelope를 영속 notification inbox로 projection |
| `WorkerPubSubOutboxEventDispatcher` | `CloudSharp.Api.Outbox` | `Target = Worker`. `FileFinalized` 등 worker 대상 이벤트를 Redis Pub/Sub topic으로 publish |
| `OutboxEventRouteRegistry` | `CloudSharp.Api.Outbox` | eventType별 dispatch target 조합을 결정 |
...
- `FileTagsAdded`, `FileTagsReplaced`, `FileTagsRemoved`
...
- `aggregateType`은 `Space`, `SpaceMember`, `SpaceInvite`, `FileItem`, `Tag`, `Folder`, `ShareLink`, `UploadSession` 중 하나다.
- 태그 생성/수정/삭제 이벤트의 `aggregateType`은 `Tag`다. 파일 태그 변경 이벤트는 대상 파일을 aggregate로 보므로 `aggregateType = FileItem`이다.
...
| `TagCreated` | `Tag` | `TagEventPayload` | Space active members | Space 태그 생성 |
| `TagUpdated` | `Tag` | `TagEventPayload` | Space active members | Space 태그 수정 |
| `TagDeleted` | `Tag` | `TagEventPayload` | Space active members | Space 태그 삭제 |
| `FileTagsAdded` | `FileItem` | `FileTagsChangedPayload` | Space active members | 파일 태그 추가 |
| `FileTagsReplaced` | `FileItem` | `FileTagsChangedPayload` | Space active members | 파일 태그 전체 교체 |
| `FileTagsRemoved` | `FileItem` | `FileTagsChangedPayload` | Space active members | 파일 태그 제거 |
...
`FileFinalized`는 현재 `Worker` target 전용 이벤트다. 후처리 worker가 썸네일/메타데이터 작업을 시작하도록 Redis Pub/Sub에 publish되며, SSE `event`로는 발행하지 않는다.
...
`TagCreated`, `TagUpdated`, `TagDeleted`에서 사용한다.
...
| `spaceId` | `number` | Space ID |
| `name` | `string` | 표시 태그명 |
| `normalizedName` | `string` | 중복 검사용 정규화 태그명 |
| `color` | `string` | `#RRGGBB` 색상 |
| `createdByUserId` | `number` | 태그 생성자 user ID |
| `createdAt` | `string(date-time)` | 생성 시각 |
| `updatedAt` | `string(date-time)` | 변경 시각 |
| `deletedAt` | `string(date-time)?` | 삭제 시각. 삭제되지 않았으면 `null` |

### 9.10 FileTagsChangedPayload

`FileTagsAdded`, `FileTagsReplaced`, `FileTagsRemoved`에서 사용한다.

| 필드 | 타입 | 설명 |
|------|------|------|
| `fileItemId` | `number` | FileItem ID |
| `spaceId` | `number` | Space ID |
| `requestedTagIds` | `number[]` | 요청에 포함된 tag ID 목록. 중복은 제거된 상태 |
| `currentTagIds` | `number[]` | 변경 후 파일에 붙어 있는 tag ID 목록 |
| `changedByUserId` | `number` | 변경 요청자 user ID |
| `changedAt` | `string(date-time)` | 변경 시각 |

### 9.11 ShareLinkEventPayload
...
builder.Services.AddScoped<IOutboxEventDispatcher, WorkerPubSubOutboxEventDispatcher>();
builder.Services.AddScoped<IOutboxEventDispatcher, NotificationProjectionOutboxEventDispatcher>();
```
...
### 12.2 Worker dispatcher

Worker dispatcher는 `WorkerPubSubOutboxEventDispatcher`와 `WorkerJobDispatchUseCases`로 구현되어 있다. 현재 `FileFinalized`는 `Worker` target으로 라우팅되고, `WorkerPubSub:Enabled = true`일 때 `cloudsharp:worker:jobs` topic으로 worker job payload를 publish한다. `FileUploaded`는 사용자 UI 갱신과 inbox projection을 위해 `RealtimeFanout | NotificationProjection`으로 라우팅한다.
...
- route registry: Tag/FileTags 이벤트는 `RealtimeFanout`, `FileFinalized`는 `Worker`, notification 대상 이벤트는 `NotificationProjection` 포함 여부 검증
- outbox processing: 여러 target이 있는 이벤트는 필요한 dispatcher가 모두 등록된 경우에만 claim 가능
