...
export interface RealtimeTagEventPayload {
  tagId: number;
  spaceId: number;
  name: string;
...
export interface RealtimeFileTagsChangedPayload {
  fileItemId: number;
  spaceId: number;
...
전체 payload schema는 `sse-realtime-fanout.md`를 기준으로 추가한다.
...
| `TagCreated` | 태그 목록 refetch |
| `TagUpdated` | 태그 목록, 현재 폴더 목록/search 결과 refetch |
| `TagDeleted` | 태그 목록, 현재 폴더 목록/search 결과 refetch |
| `FileTagsAdded` | 현재 폴더 목록 및 파일 상세 refetch |
| `FileTagsReplaced` | 현재 폴더 목록 및 파일 상세 refetch |
| `FileTagsRemoved` | 현재 폴더 목록 및 파일 상세 refetch |
...
function handleRealtimeEvent(event: RealtimeEventEnvelope) {
  switch (event.eventType) {
    ...
    case 'FileDeleted':
    case 'FileTagsAdded':
    case 'FileTagsReplaced':
    case 'FileTagsRemoved':
    case 'FolderCreated':
    case 'FolderRenamed':
    case 'FolderMoved':
    case 'FolderDeleted':
